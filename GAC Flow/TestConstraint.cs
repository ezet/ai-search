...
namespace eZet.Csp.Flow {
    public class TestConstraint : IConstraint {
        ...
        public TestConstraint(IConnectedVariable centerVariable, IEnumerable<IConnectedVariable> neighbours) {
        ...
        public IEnumerable<IDomainValue> Eval2(IVariable focusVariable) {
            var focus = (IConnectedVariable)focusVariable;

            var connectableNeighbours =
                _neighbours.Where(
                    n =>
                        n.DomainValues.Any(
                            v =>
                                _centerVariable.DomainValues.Cast<FlowGridDomainValue>()
                                    .Any(d => d.Value == ((FlowGridDomainValue) v).Value))).ToList();
            var inputs =
                connectableNeighbours.Where(n => n.DomainValues.Any(v => ((FlowGridDomainValue) v).Out == _centerVariable));
            var outputs =
             connectableNeighbours.Where(n => n.DomainValues.Any(v => ((FlowGridDomainValue)v).In == _centerVariable));
